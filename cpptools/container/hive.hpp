// Copyright (c) 2022, Matthew Bentley (mattreecebentley@gmail.com) www.cpptoolslib.org
...
// Copied from https://github.com/mattreecebentley/cpptools_hive, part the author's
// proposal of a new container to be introduced in C++23.

// Some namespaces were renamed throughout this file for ease of use within the
// library.

#ifndef CPPTOOLS__CONTAINER__HIVE_HPP
#define CPPTOOLS__CONTAINER__HIVE_HPP
...
template <class element_type, class allocator_type = std::allocator<element_type>, cpptools::hive_priority priority = cpptools::hive_priority::performance> class hive : private allocator_type // Empty base class optimisation (EBCO) - inheriting allocator functions
{
	...
	typedef typename choose<priority == cpptools::hive_priority::performance, unsigned short, unsigned char>::type		skipfield_type; // Note: unsigned short is equivalent to uint_least16_t ie. Using 16-bit unsigned integer in best-case scenario, greater-than-16-bit unsigned integer where platform doesn't support 16-bit types. unsigned char is always == 1 byte, as opposed to uint_8, which may not be
...
private:
	...
	{
		return static_cast<skipfield_type>((sizeof(aligned_element_type) * 8 > (sizeof(cpptools::hive<element_type>) + sizeof(group)) * 2) ?
			8 : (((sizeof(cpptools::hive<element_type>) + sizeof(group)) * 2) / sizeof(aligned_element_type)));
	}


	inline void check_capacities_conformance(cpptools::hive_limits capacities) const
	{
...
public:
	...
	explicit hive(const cpptools::hive_limits capacities):
		allocator_type(allocator_type()),
		groups_with_erasures_list_head(NULL),
		unused_groups_head(NULL),
		total_size(0),
	...
	explicit hive(const cpptools::hive_limits capacities, const allocator_type &alloc):
		allocator_type(alloc),
		groups_with_erasures_list_head(NULL),
		unused_groups_head(NULL),
		total_size(0),
...
public:
	...
	hive(const size_type fill_number, const element_type &element, const cpptools::hive_limits capacities = cpptools::hive_limits(get_minimum_block_capacity(), std::numeric_limits<skipfield_type>::max()), const allocator_type &alloc = allocator_type()):
		allocator_type(alloc),
		groups_with_erasures_list_head(NULL),
		unused_groups_head(NULL),
		total_size(0),
	...
	explicit hive(const size_type fill_number, const cpptools::hive_limits capacities = cpptools::hive_limits(get_minimum_block_capacity(), std::numeric_limits<skipfield_type>::max()), const allocator_type &alloc = allocator_type()):
		allocator_type(alloc),
		groups_with_erasures_list_head(NULL),
		unused_groups_head(NULL),
		total_size(0),
	...
	hive(const typename std::enable_if_t<!std::numeric_limits<iterator_type>::is_integer, iterator_type> &first, const iterator_type &last, const cpptools::hive_limits capacities = cpptools::hive_limits(get_minimum_block_capacity(), std::numeric_limits<skipfield_type>::max()), const allocator_type &alloc = allocator_type()):
		allocator_type(alloc),
		groups_with_erasures_list_head(NULL),
		unused_groups_head(NULL),
		total_size(0),
	...
	hive(const iterator_type &first, const sentinel &last, const cpptools::hive_limits capacities = cpptools::hive_limits(get_minimum_block_capacity(), std::numeric_limits<skipfield_type>::max()), const allocator_type &alloc = allocator_type()):
		allocator_type(alloc),
		groups_with_erasures_list_head(NULL),
		unused_groups_head(NULL),
		total_size(0),
	...
	hive(const std::initializer_list<element_type> &element_list, const cpptools::hive_limits capacities = cpptools::hive_limits(get_minimum_block_capacity(), std::numeric_limits<skipfield_type>::max()), const allocator_type &alloc = allocator_type()):
		allocator_type(alloc),
		groups_with_erasures_list_head(NULL),
		unused_groups_head(NULL),
		total_size(0),
...
private:
	...
	void consolidate()
	{
		...
		{
			hive temp(cpptools::hive_limits(group_allocator_pair.min_group_capacity, aligned_allocator_pair.max_group_capacity));
...
public:
	...
	void reshape(const cpptools::hive_limits capacities)
	{
		check_capacities_conformance(capacities);
	...
	inline cpptools::hive_limits block_limits() const noexcept
	{
		return cpptools::hive_limits(static_cast<size_t>(group_allocator_pair.min_group_capacity), static_cast<size_t>(aligned_allocator_pair.max_group_capacity));
	}
...
namespace std
{
	...
	template <class element_type, class allocator_type, cpptools::hive_priority priority>
	inline void swap (cpptools::hive<element_type, allocator_type, priority> &a, cpptools::hive<element_type, allocator_type, priority> &b) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_swap::value || std::allocator_traits<allocator_type>::is_always_equal::value)
	{
		a.swap(b);
	}
	...
	template <class element_type, class allocator_type, cpptools::hive_priority priority, class predicate_function>
	typename cpptools::hive<element_type, allocator_type, priority>::size_type erase_if(cpptools::hive<element_type, allocator_type, priority> &container, predicate_function predicate)
	{
		typedef typename cpptools::hive<element_type, allocator_type, priority> hive;
	...
	template <class element_type, class allocator_type, cpptools::hive_priority priority>
	inline typename cpptools::hive<element_type, allocator_type, priority>::size_type erase(cpptools::hive<element_type, allocator_type, priority> &container, const element_type &value)
	{
		return erase_if(container, cpptools::hive_eq_to<element_type>(value));
	}
}


#endif // CPPTOOLS__CONTAINER__HIVE_HPP